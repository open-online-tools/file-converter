...
function checkVersion() {
  ...
  if (versionDiff) {
    ...
    console.error('');
    console.error('Version changes in package.json are prohibited in pull requests.');
    console.error('Versions are managed automatically by the CI/CD pipeline using changesets.');
    console.error('');
    console.error('To request a release:');
    console.error('  1. Add a changeset file describing your changes (npx changeset)');
    console.error('  2. The release workflow will automatically bump the version when merged');
    console.error('');
